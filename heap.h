...
/* 用新节点取代旧节点 */
static inline void __heap_replace(struct heap_head *head, struct heap_node *oldn, struct heap_node *newn) {
    struct heap_node *parent, *left, *right;
    parent = oldn->parent;
    left = oldn->left;
    right = oldn->right;
    ...
    if (left) left->parent = newn;
    if (right) right->parent = newn;
...
/* 父子节点进行交换 */
static inline void __heap_swap(struct heap_head *head, struct heap_node *parent, struct heap_node *child) {
    assert(parent);
